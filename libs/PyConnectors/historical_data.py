...
def savedata(_path,_data,_ticker,_end_date,_time_duration,_stick_duration,_header,_filename):
    if(len(_data)==0):
...
tickers = ['NIO','SUNW','EBAY','DKNG','PLUG','X','QQQ','SPY']
end_date = datetime.datetime(2021, 2, 4, 23, 59, 59)
...
for ticker in tickers:
    for i in range(0,len(candlestick_durations)):
        try:
          filename = ticker+'-'+end_date.strftime("%Y-%m-%d-%H-%M-%S")+"-"+time_durations[i].name+'-'+candlestick_durations[i].name+'.csv'  
          data = ibkr.historicdata(ticker,time_durations[i],candlestick_durations[i],end_date=end_date,include_non_trading_hours=True)
          savedata(path,data,ticker,end_date,time_durations[i],candlestick_durations[i],header,filename)
        except:
